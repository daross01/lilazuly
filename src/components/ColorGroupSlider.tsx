      ...
      <div className="relative group">
        ...
        >
          {visibleImages.map((image) => {
            ...
            const pinMedia = image.src.startsWith("http") ? image.src : `${DOMAIN}${image.src}`;
            return (
              <div
                key={image.id}
                className="relative flex-shrink-0 w-40 md:w-52 snap-start group/card"
              >
                <PhoneMockup
                  src={image.src}
                  alt={image.alt}
                  ...
                  topRightSlot={
                    <a
                      ...
                      className="w-7 h-7 rounded-full bg-background/80 backdrop-blur-sm flex items-center justify-center opacity-0 group-hover/card:opacity-100 transition-opacity shadow-card"
